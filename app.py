...
def filter_by_year(statistics, year, yearid):
    ...
    for batting_stats in statistics:
        if batting_stats.get(yearid) == year:
            results.append(batting_stats)
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for player_stat in statistics:
        # An error from 2016 master batting file
        if isinstance(player_stat, dict):
            player_id = player_stat.get(info.get("playerid"))
            compound_stat = formula(info, player_stat)
            player_list.append((player_id, compound_stat))

    results = sorted(player_list, reverse=True, key=lambda x: x[1])

    return results[0:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    table = read_csv_as_nested_dict(info.get("masterfile"),
                                    info.get('playerid'), info.get('separator'), info.get('quote'))
    players = []
    for player in top_ids_and_stats:
        player_id = player[0]
        compound_stat = player[1]
        row = table.get(player_id)
        first_name = row.get(info.get('firstname'))
        last_name = row.get(info.get('lastname'))

        players.append('{0:.3f} --- {1} {2}'.format(compound_stat, first_name, last_name))
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    yearly_batting_stats = read_csv_as_list_dict(info.get('battingfile'),
                                                 info.get('separator'), info.get('quote'))
    year_batting_stat = filter_by_year(yearly_batting_stats, year, info.get('yearid'))
    players = top_player_ids(info, year_batting_stat, formula, numplayers)

    return lookup_player_names(info, players)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for data in statistics:
        ...
        if player not in aggregate_data:
            ...
            aggregate_data[player][playerid] = player

        player_stat = aggregate_data.get(player)

        for field in fields:
            if field not in player_stat:
                player_stat[field] = int(data.get(field))
            else:
                total = int(player_stat[field]) + int(data.get(field))
                player_stat[field] = total
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    players = read_csv_as_list_dict(info.get('masterfile'), info.get('separator'), info.get('quote'))
    bats = read_csv_as_list_dict(info.get('battingfile'), info.get('separator'), info.get('quote'))
    players_stats = aggregate_by_player_id(bats, info.get('playerid'), info.get('battingfields'))
    batting_stats = []

    player_id = info.get('playerid')

    for player in players:
        player_stat = players_stats.get(player.get(player_id))
        batting_stats.append(player_stat)

    top_player_stats = top_player_ids(info, batting_stats, formula, numplayers)
...
def test_baseball_statistics():
    ...
    #
    baseballdatainfo = {"masterfile": "data/Master_2016.csv",   # Name of Master CSV file
                        "battingfile": "data/Batting_2016.csv", # Name of Batting CSV file
